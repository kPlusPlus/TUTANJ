...
for($j=1;$j<$i;$j++)  
{
	...
	try {				
		//$surl = "https://coinmarketcap.com/currencies/shiba-inu/"; // TEST TEST TEST TEST
		$surl = "https://www.coingecko.com/en/coins/shiba-inu";
		$page = file_get_contents($surl);
		echo $page;
		file_put_contents('getshinu.html',$page);
		exit;
		if($page === false)	{
	...
	preg_match('/priceValue___11gHJ smallerPrice___1oph9">\$(\d{0,10}.\d{0,10})/', $page,$matches,PREG_OFFSET_CAPTURE);
	...
	{	
		...
		echo "\e[42m". "[diff]" . str_pad(" ", $str - 6 - (strlen(round($differ,2)))," ",STR_PAD_LEFT) . number_format($differ,10) . "$ " . $perc . " " . (($perc * 100)-100) . "%". "\e[0m". PHP_EOL;
	...
	{
		echo str_repeat("_", 70).PHP_EOL;
		echo "\e[1;37;41m". "[diff] " . str_pad(" ", $str - 7  - (strlen(round($differ,2)))," ",STR_PAD_LEFT) . number_format($differ,10) . "$ " . $perc . " " . (($perc * 100)-100) . "%". "\e[0m".PHP_EOL;	
